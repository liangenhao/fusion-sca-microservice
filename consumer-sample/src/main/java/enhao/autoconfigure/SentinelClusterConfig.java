...
import enhao.consumer.config.dto.ClusterAppAssignMap;
...
@ConditionalOnProperty(name = "sentinel-cluster.enabled", havingValue = "true")
@EnableConfigurationProperties(SentinelClusterProperties.class)
public class SentinelClusterConfig {

    private final SentinelClusterProperties properties;

    public SentinelClusterConfig(SentinelClusterProperties properties) {
                ...
                source -> {
                    ...
                    return Optional.ofNullable(assignMaps)
                            .flatMap(assignMapList -> assignMapList.stream()
                                    ...
                                    .findAny()
                                    .map(ClusterAppAssignMap::getNamespaceSet))
                            .orElse(new HashSet<>());
                });
    ...
    private Optional<ServerFlowConfig> extractServerFlowConfig(List<ClusterAppAssignMap> groupList) {
        return groupList.stream()
                .filter(this::machineEqual)
                .findAny()
                .map(e -> new ServerFlowConfig().setMaxAllowedQps(e.getMaxAllowedQps()));
    }
